...
/* view of current delta, derived from current full */
	...
	create  index idx_effectivetime on temp_table(effectivetime);
	create  index idx_active on temp_table(active);
	create  index idx_moduleid on temp_table(moduleid);
	create  index idx_sourceid on temp_table(sourceid);
	create  index idx_destinationid on temp_table(destinationid);
	create  index idx_relationshipgroup on temp_table(relationshipgroup);
	create  index idx_typeid on temp_table(typeid);
	create  index idx_characteristictypeid on temp_table(characteristictypeid);
	create  index idx_modifierid on temp_table(modifierid);
